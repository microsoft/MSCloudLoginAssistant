            ...
            if ($null -eq $Global:OpenExchangeSession) {
                try {
                    ...
                    while ($null -eq $Global:ExchangeOnlineSession) {
                        ...
                        try {
                            $Global:ExchangeOnlineSession = New-PSSession @ExchOnlineParams
                        }
                        catch {
                            try {
                                ...
                                $ExchOnlineParams["ConnectionURI"] = "https://outlook.office365.com/PowerShell-LiveId?BasicAuthToOAuthConversion=true"
                                $ExchOnlineParams["Credential"] = $Ctoken
            ...
            if ($null -eq $Global:SessionSecurityCompliance) {
                try {
                    try {
                        ...
                        $Global:SessionSecurityCompliance = New-PSSession @SecurityComplianceParams
                    }
                    catch {
                        Write-Verbose -Message "Session to Security & Compliance no working session found, creating a new one"
